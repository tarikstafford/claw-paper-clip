...
- Create an agent: POST to /api/companies/$PAPERCLIP_COMPANY_ID/agents with {"name": "...", "adapterType": "opencode_local", "adapterConfig": {"model": "..."}}
- Create an issue: POST to /api/companies/$PAPERCLIP_COMPANY_ID/issues with {"title": "...", "description": "..."}
...
Your final text output will be automatically posted back to the chat thread — do NOT post it yourself.`
    : "";
  // In chat mode, put the conversation context and company info first so the model
  // focuses on the chat rather than getting lost in 20K+ chars of skill content.
